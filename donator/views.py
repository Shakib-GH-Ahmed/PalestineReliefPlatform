...
def donator_profile(request):
    ...
    checker = PersonTable.objects.filter(
        PersonType="D", EMAIL=email, PASSWORD=password)
    return render(request, 'donator/donator_profile.html', {'checker': checker})
        ...
        for event in all_events:
            for fund in all_events_fund:
                if fund.FUND_STATUS == "Not Fulfilled" and 'E'+str(event.id) == str(fund.EVENT_ID):
                    all_events_dict[f'E{event.id}'] = [fund.EVENT_ID, event.EVENT_NAME, event.LOCATION, event.BUDGET, int(
                        event.BUDGET)-int(fund.COLLECTED_AMOUNT)]
        if request.method == 'POST':
            data = request.POST
            ...
            if int(amount) > int(selected_event[0].BUDGET) - int(selected_event_fund[0].COLLECTED_AMOUNT):
                ...
                total_amount = int(amount) + \
                    int(selected_event_fund[0].COLLECTED_AMOUNT)
                if total_amount == int(selected_event[0].BUDGET):
                    status = "Fulfilled"
                else:
                    status = "Not Fulfilled"
                selected_event_fund.update(
                    COLLECTED_AMOUNT=total_amount, FUND_STATUS=status)
                d_id = PersonTable.objects.filter(
                    PersonType="D", EMAIL=email, PASSWORD=password)
                donate_table = DonateTable(
                    D_ID='D'+str(d_id[0].id), EVENT_ID=id_e, PAYMENT_DETAILS='bKash', PAYMENT=amount)
                donate_table.save()
                messages.success(request, 'Success')
                checker = PersonTable.objects.filter(
                    PersonType="D", EMAIL=email, PASSWORD=password)
