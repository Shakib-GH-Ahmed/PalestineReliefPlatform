    ...
    if request.method == 'POST':
        ...
        if data.is_valid():
            ...
            if duplicate_checker == []:
                ...
                volunteer_table = VolunteerTable(
                    V_ID=p_id, EMAIL=email, STATUS='Not in volunteering', VOLUNTARY='Null')
                person_table.save()
...
def volunteer_profile(request):
    ...
    checker = PersonTable.objects.filter(
        PersonType="V", EMAIL=email, PASSWORD=password)
    getData = VolunteerTable.objects.filter(EMAIL=email)
    return render(request, 'volunteer/volunteer_profile.html', {'checker': checker, 'getData': getData})
...
def event_details_v(request):
    if request.session.has_key('EMAIL') and request.session.has_key('PASSWORD'):
        ...
        for event in all_events:
            all_events_dict[f'E{event.id}'] = [
                'E'+str(event.id), event.EVENT_NAME, event.END_TIME, event.LOCATION]
        if request.method == 'POST':
            data = request.POST
            for keys, values in data.items():
                if keys == 'id':
                    id_e = values
            selected_voluntary_event = VolunteerTable.objects.filter(
                EMAIL=email)
            if selected_voluntary_event[0].STATUS == 'Not in volunteering':
                selected_voluntary_event.update(
                    STATUS='In volunteering', VOLUNTARY=id_e)
            else:
                return render(request, "home/error.html")
