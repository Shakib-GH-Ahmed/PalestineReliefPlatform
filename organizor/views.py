...
def SignupOrganizor(request):
    ...
    if request.method == 'POST':
        ...
        if data.is_valid():
            ...
            duplicate_checker = list(PersonTable.objects.filter(
                PersonType=p_id, EMAIL=email, NID=nid))
            if duplicate_checker == []:
                person_table = PersonTable(PersonType=p_id, FIRST_NAME=first_name,
                                           LAST_NAME=last_name, EMAIL=email, CONTACT=contact, NID=nid, PASSWORD=password)
        ...
        return redirect('LoginOrganizor')
...
def LoginOrganizor(request):
    ...
    if request.method == 'POST':
        ...
        if data.is_valid():
            ...
            email = data.cleaned_data['EMAIL']
            password = data.cleaned_data['PASSWORD']
            checker = PersonTable.objects.filter(
                PersonType=p_id, EMAIL=email, PASSWORD=password)
            if list(checker) != []:
...
def organizor_profile(request):
    ...
    checker = PersonTable.objects.filter(
        PersonType="O", EMAIL=email, PASSWORD=password)
    return render(request, 'organizor/organizor_profile.html', {'checker': checker})
    ...
    if request.method == 'POST':
        ...
        if request.session.has_key('EMAIL') and request.session.has_key('PASSWORD'):
            ...
            for keys, values in data.items():
                ...
                if keys == 'Event_For':
                    Event_For = values
            checker = EventTable.objects.filter(
                EVENT_NAME=Event_Name, LOCATION=Location, ITEMS=Items, EVENT_FOR=Event_For)
            if list(checker) == []:
                event_table = EventTable(EVENT_NAME=Event_Name, START_TIME=Start_Time, END_TIME=End_Time, LOCATION=Location,
                                         BUDGET=Budget, SHOP=Shop, ITEMS=Items, QUANTITY=Quantity, EVENT_FOR=Event_For, CREATED_BY=email)
                shop_table = ShopTable(
                    SHOP_NAME=Shop, PAY_DEMAND=Budget, DELIVERY_STATUS='Pending')
                event_table.save()
                shop_table.save()
                data = EventTable.objects.filter(CREATED_BY=email)
                for d in data:
                    fund_table = FundTable(EVENT_ID='E'+str(d.id), COLLECTED_AMOUNT=0,
                                           FUND_STATUS="Not Fulfilled", FUND_TAKEN_BY='Not Taken')
                    fund_table.save()
                messages.success(request, 'Success')
            else:
                return render(request, 'home/error.html')
